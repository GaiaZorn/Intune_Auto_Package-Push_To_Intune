...
A window will appear. Drag and drop an `.exe` or `.msi` file onto the window. The script will create a zipped copy of the installer, detect silent install and uninstall commands and then upload the metadata to your Intune tenant using the device flow authentication.
...
## Download as Zip for Windows

Each time you drop a file onto the application window, the tool creates a `.zip`
archive next to the original installer. You can use this archive on Windows or
upload it to Intune directly.
